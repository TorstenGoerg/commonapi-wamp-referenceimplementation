...
static void add2(autobahn::wamp_invocation invocation)
{
    auto s = invocation->argument<Summands>(0);

    std::cerr << "Procedure com.examples.calculator.add2 invoked: " << s.left << ", " << s.right << std::endl;

    SumDiff sd = {s.left + s.right, s.left - s.right};
    invocation->result(std::make_tuple(sd));
//    invocation->result(std::make_tuple(s.left + s.right));
